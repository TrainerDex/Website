...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            key = (
                Site.objects.get(domain=options["site"][0])
                .socialapp_set.filter(provider="discord")
                .first()
                .key
            )
        except:
            raise CommandError(
                "Can't find key for site '{}'".format(options["site"][0])
            )
        ...
        async def on_ready():
            ...
            channel = client.get_channel(
                guilddb.discordguildsettings.monthly_gains_channel.id
            )
            guilddb.refresh_from_api()
            guilddb.sync_roles()
            guilddb.sync_members()
            _unfiltered_members = DiscordGuildMembership.objects.filter(
                active=True, guild=guilddb
            )
            _unfiltered_members = [
                x
                for x in _unfiltered_members
                if "NoLB" not in (y.name for y in x.roles)
            ]
            mbrs = filter_leaderboard_qs(
                Trainer.objects.filter(
                    owner__socialaccount__discordguildmembership__in=_unfiltered_members
                )
            )
            ...
            this_months_submissions = (
                Update.objects.filter(
                    trainer__in=mbrs,
                    update_time__lte=this_month,
                    update_time__gt=this_month + relativedelta(weeks=-1),
                )
            ...
            print("Loading updates on latest date", end="")
            last_months_submissions = (
                Update.objects.filter(
                    trainer__in=mbrs,
                    update_time__lte=last_month,
                    update_time__gt=last_month + relativedelta(weeks=-1),
                )
                .order_by("trainer", "-update_time")
                .distinct("trainer")
            )
            ...
            eligible_entries = this_months_submissions.filter(
                trainer__in=last_months_submissions.values_list("trainer", flat=True)
            )
            print("Eligible Entries: ", eligible_entries.count())
            new_entries = this_months_submissions.exclude(
                trainer__in=last_months_submissions.values_list("trainer", flat=True)
            )
            print("New Entries: ", new_entries.count())
            dropped_trainers = last_months_submissions.exclude(
                trainer__in=this_months_submissions.values_list("trainer", flat=True)
            )
                ...
                gains_list.append(
                    {
                        ...
                        "gain": entry.total_xp
                        - last_months_submissions.get(trainer=entry.trainer).total_xp,
                        "delta": entry.update_time.date()
                        - last_months_submissions.get(
                            trainer=entry.trainer
                        ).update_time.date(),
                    }
                )
            gains_list.sort(
                key=lambda x: x["new"].total_xp - x["old"].total_xp, reverse=True
            )
            ...
            for a, b in enumerate(gains_list):
                leaderboard_text += "#{pos} **{trainer}** gained {gain}! (_{old_xp}⇒{new_xp}_, _{old_date}⇒{new_date}_, **{delta}**)\n".format(
                    pos=a + 1,
                    trainer=b["trainer"].nickname,
                    gain=format_number(b["gain"]),
                    old_xp=format_number(b["old"].total_xp),
                    new_xp=format_number(b["new"].total_xp),
                    delta=format_timespan(b["delta"], max_units=2),
                    old_date=naturaldate(b["old"].update_time),
                    new_date=naturaldate(b["new"].update_time),
                )

            if new_entries.count() > 0:
                leaderboard_text += "\n\nWe have **{x}** new entries who may be ranked next week, including; _{top_5}_".format(
                    x=new_entries.count(),
                    top_5=", ".join(
                        [
                            x.trainer.nickname
                            for x in sorted(
                                new_entries, key=lambda x: x.total_xp, reverse=True
                            )[:5]
                        ]
                    ),
                )
            ...
            if dropped_trainers.count() > 0:
                leaderboard_text += "\n**{x}** trainers didn't submit in time this week so couldn't be ranked, including; _{top_5}_".format(
                    x=dropped_trainers.count(),
                    top_5=", ".join([x.trainer.nickname for x in dropped_trainers[:5]]),
                )
