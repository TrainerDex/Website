...
from trainerdex.api.v1.serializers import BriefTrainerSerializer, BriefUpdateSerializer, DetailedTrainerSerializer, DetailedUpdateSerializer, LeaderboardSerializer, SocialAllAuthSerializer, UserSerializer
...
class LatestUpdateView(APIView):
    ...
    def patch(self, request, pk):
        update = Update.objects.filter(trainer=pk, trainer__owner__is_active=True).latest('update_time')
        if update.meta_time_created > datetime.datetime.utcnow()-datetime.timedelta(minutes=32):
...
class UpdateDetailView(APIView):
    ...
    def delete(self, request, uuid, pk):
        update = get_object_or_404(Update, trainer=pk, uuid=uuid, trainer__owner__is_active=True)
        if update.meta_time_created > datetime.datetime.utcnow()-datetime.timedelta(days=2):
            update.delete()
